...
{
	std::array<const char*, 2> choices = { "Run Driver", "Settings" };
	...
	while (!done)
	{
		switch(WZ::AskInput(choices))
		{
		case 1:
			std::cout << "\n";
			WZ::GameManager::gameStart();
			done = true;
			break;

		case 2:
			std::cout << "\n";
			WZ::GameManager::SettingsMenu();
			break;
		}
	...
	std::cout << "Map: \n" << *WZ::GameManager::getMap();

	std::cout << "\n\nPlayers:\n";

	for (WZ::Player* p : WZ::GameManager::getActivePlayers())
	{
		std::cout << p->getPlayerName() << "\n";
	}

	std::cout << "running a game with current observer settings (map and players will change): \n\n\n";
